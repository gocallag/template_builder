...
try:
    ...
    with open('{{ imagebuilder_build_imagedir }}/{{ imagebuilder_id_base }}.x86_64.qcow2', 'rb') as image_file:
    ...
    print(f"Finalizing upload for disk '{{ imagebuilder_id_base }}_disk'...")
    ...
    print(f"Upload and finalize completed for '{{ imagebuilder_id_base }}_disk'")
    # sleep(20)  # Extra wait to ensure everything is settled


    # Create or update the template

    print(f"Creating new VM '{{ imagebuilder_id_base }}_tmpvm' before turning it into a template...")
    vms_service = connection.system_service().vms_service()
    ...
    print(f"Disk '{{ imagebuilder_id_base }}_disk' attached to VM '{tempvm_name}'."))
